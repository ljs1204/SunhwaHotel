...
// 20220314 예약 상세보기 START - SI
	...
	public String myReserveDetail(Model model, @RequestParam String reserve_num, @RequestParam int reserve_idx,
			HttpSession session) {
...
// 20220319 환불 신청 페이지 START - SI
	...
	public String myReserveRefund(Model model, @RequestParam String reserve_num, HttpSession session) {
...
// 20220320 구매한 마일리지 상품 확인 페이지 END - SI
	...
	public String mypageRefundDetail(Model model, @RequestParam String reserve_num, HttpSession session) {
		logger.info("mypage2Reserve 환불현황상세 ");

		// 아이디
		String userId = (String) session.getAttribute("loginId");
		//logger.info(userId);
		//logger.info(reserve_num);

		//MypageDTO reserDto = new MypageDTO();

		if(userId != null) {
			ArrayList<ReserveDTO> refundReserve = mypageService.reserInfo(userId, reserve_num);
			logger.info("refundReserve : {}", refundReserve.size());
			model.addAttribute("refundReserve", refundReserve);
		}else {
			ArrayList<ReserveDTO> refundReserve = mypageService.reserInfo(userId, "비회원");
			logger.info("refundReserve : {}", refundReserve.size());
			model.addAttribute("refundReserve", refundReserve);
		}
		
		ArrayList<ReserveDTO> refundPay = mypageService.payDto(reserve_num); 
		model.addAttribute("refundPay", refundPay);
	...
	public String myPagemymilelist(Model model, HttpSession session, @RequestParam(value = "orderNum") int orderNum,
			MemberDTO memDto) {
		...
		ArrayList<MemberDTO> milelist = mypageService.milelistInfo(milelistPage.getDisplayPost(),
				milelistPage.getPostNum(), loginId);
	 ...
	 * 
	 * @GetMapping(value="/tomemberboardlist") public ModelAndView
	 * tomemberboardlist(HttpSession session) {
	 * 
	 * String loginId = (String) session.getAttribute("loginId"); // 유선화 문의 리스트 세션처리
	 * 하기 String mem_grade = (String) session.getAttribute("mem_grade");
	 * 
	 * ModelAndView v = null; //if(loginId != null && mem_grade != "admin") {
	 * 
	 * v = mypageService.tomemberboardlist(loginId); //}
	 * 
	 * ModelAndView mv = new ModelAndView(); mv.addObject("loginId" , loginId);
	 * 
